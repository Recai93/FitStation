...
import com.udacity.firebase.shoppinglistplusplus.model.DailyMeal2;
...
//        String arrayStr = "private static final String[] tips = {";
//        for (String str : array) {
//            arrayStr += "\""+str.trim()+"\",";
//        }
//        arrayStr+="};";
//        Log.i("","");Log.i("**********", arrayStr);
//    }
...
//    }
    ...
    public class FetchNextVenuesTask extends AsyncTask<String, Void, String[]> {
        ...
        protected String[] doInBackground(String... params) {
            Document doc = null;
            List<DailyMeal2> mealList = new ArrayList<>();
            try {
                doc = Jsoup.connect("http://www.antrenmanyap.com/tr/blog.asp?ID=6&CID=7&AID=20&do=show").get();
            } catch (IOException e) {
                e.printStackTrace();
            }

            if (doc != null) {
                Elements tables = doc.select(".content-text table");
                for (Element tableEl : tables) {
                    ...
                    for (Element trEl : trEls) {
                        ...
                        String mealName = tdEls.get(0).text().toLowerCase();
                        String calorie = tdEls.get(1).text();
                        DailyMeal2 meal = new DailyMeal2(mealName, calorie);
                        mealList.add(meal);
                    }
                }
            }
...
//                        mealList.add(meal);
        ...
        private void saveToDB(List<DailyMeal2> meals) {
            Firebase firebaseRef = new Firebase(Constants.FIREBASE_URL + "/meals");
            for (DailyMeal2 meal : meals) {
                Firebase ref = firebaseRef.push();
                String pushId = ref.getKey();
                firebaseRef.child(pushId).setValue(meal);
            }
